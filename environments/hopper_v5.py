...
class HopperEnv(mujoco_env.MujocoEnv, utils.EzPickle):
    ...
    def step(self, a):
        ...
        alive_bonus = 1.0
        reward_energy_1 = 4.0 - 3.0 * np.square([a[0]]).mean() + alive_bonus
        reward_energy_2 = 4.0 - 3.0 * np.square([a[1]]).mean() + alive_bonus
        reward_energy_3 = 4.0 - 3.0 * np.square([a[2]]).mean() + alive_bonus
        ...
        #     done = True
        ob = self._get_obs()
        return ob, np.array([reward_run, reward_jump, reward_energy_1, reward_energy_2, reward_energy_3]), done, {'obj': np.array([reward_run, reward_jump,  reward_energy_1, reward_energy_2, reward_energy_3])}
