...
def run():
    ...
    name = "Motivation_Regular_other_"+str(args.consider_other)+"_bar_"+ str(args.regular_bar)+"_Freq_"+str(args.old_Q_update_freq)+"_Regular_" + str(args.regular_alpha)+"_Hardupdate_Use_PC_"+str(args.Use_pc_grad) + "_step_random_"  +  str(args.step_random)+"_"+ str(args.use_encoder_hardupdate)+"_" + str(args.Critic_use_both) + "_"+str(args.use_avg)+"_"+str(args.Critic_use_s) + "_"+ str(args.Critic_use_a)+"_"+str(args.Policy_use_latent)+str(args.Policy_use_s)+"_"+str(args.Policy_use_w)+ "_Env_" + str(args.env_id)
    ...
    our_wandb = wandb.init(project="Motivation_MORL",name=name)
